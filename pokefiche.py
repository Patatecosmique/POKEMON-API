...
def poke_to_md(data: dict, filename: str) -> None:
    ...
    with open(filename, 'w', encoding='utf-8') as md_file:
        ...
        md_file.write(f"- **Taille**: {height}\n")
        md_file.write(f"- **Poids**: {weight}\n")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        # Affichage du message d'erreur si le nombre d'arguments est incorrect
        print("Usage: python3 pokefiche.py <pokemon_id>")
    else:
        # Génération de la fiche du pokémon
        fiche_pokemon(int(sys.argv[1]))